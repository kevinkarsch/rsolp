...
{
    register CFLOAT sum = 0, sumx = 0, sumy = 0;
    register WORD *ai, *bj;
    ai=a->words;
    bj=b->words;
    while (ai->wnum && bj->wnum) {
        if(ai->wnum > bj->wnum) {
            ...
            register CFLOAT x = (CFLOAT) (ai->weight);
            register CFLOAT y = (CFLOAT) (bj->weight);
            sum+= (x>y) ? y : x;
            sumx += x;
            sumy += y;
            ai++;
    ...
    sumx += 1e-7;
    sumy += 1e-7;
    sumx = (sumx < sumy) ? sumx : sumy;
    /* sum = exp( -kernel_parm->rbf_gamma * sum);*/
    return((double) (1- (sum / sumx)) );
}
