...
SurfaceHandler.prototype.buildToolbar = function(){
	...
	$(this.surfaces).each(function(key, value){
		...
		element.append($('<a class="delete-link">[del]</a>').click(function(){ self.deleteSurface(key); }));
		element.append("&nbsp;&nbsp;&nbsp;&nbsp;Solid:");
		if(value.solid) {
			element.append($('<input type="checkbox" checked/>').click(function(){ self.toggleSolid(key); }));
		} else {
			element.append($('<input type="checkbox"/>').click(function(){ self.toggleSolid(key); }));
		}
		list.append(element);
	});
...
SurfaceHandler.prototype.addSurface = function(){
    ...
    this.addVertexMode(this.surfaces.length-1);
...
SurfaceHandler.prototype.toggleSolid = function(key){
	this.surfaces[key].solid = 1-this.surfaces[key].solid;
}
...
SurfaceHandler.prototype.moveSurfaceMouseMoveHandler = function(e){
	...
	if(this.isMouseDown && this.dragVertex){
		if(this.dragVertex.relativeTo !== undefined){
			...
			var centroid = surface.polygon.vertices[0];
